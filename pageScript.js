...
// 语法参考vue 组件配置，唯一区别是不需要定义data属性，会自动把数据结构作为data定义
function convertLinesData(data) {
  return data.map(item => ({
    coords: [
      [item.sourceLng, item.sourceLat],
      [item.targetLng, item.targetLat],
    ],
  }));
}
function convertEffectScatterData(data) {
  return data.map(item => ({
    name: item.name,
...
const converts = {
  lines: convertLinesData,
  effectScatter: convertEffectScatterData,
};
function callback(echart,config, UI) {
  let series = [];
  Object.keys(echart.layers).forEach(key => {
    let serie = echart.layers[key];

    if (serie.show) {
      serie.data = converts[serie.type](config[serie.type].dataJson.data);
      series.push(serie);
    }
  });
  
  //需要对 geo.itemStyle.normal.areaColor进行处理

  if(echart.geo.itemStyle.normal.useAreaBackgroundImg && echart.geo.itemStyle.normal.areaBackgroundImg){
      let img = new Image();
      img.src = echart.geo.itemStyle.normal.areaBackgroundImg;
      echart.geo.itemStyle.normal.areaColor = {
        repeat: 'repeat',
        image:img
      };
  }
  if(!echart?.geo?.center?.length){
    echart.geo.center = undefined;
  }
  echart.series = series;
  if(UI.isDrill !== true){
    echart.geo.selectedMode = 'single';
  }
  return echart;
}
...
export default {
  ...
  //   echart: function (newEchart, oldEchart) {
  //     console.log('echart change');
  //     this.echart=callback(this.echart,this.config);
  //   }
  // },
  destroyed:function(){
   ...
   mounted:function(){
      //console.log('11111111111111111111mounted:',this.UI.echart.geo.map);
      let $this=this;
      $this.echart=callback($this.echart,$this.config,$this.UI);
      console.log("🏴 option", $this.echart)
     $this.echart.geo.label.rotate = 74;
      echartInstance = echarts.init(this.$refs.mapChart,null,{ height:$this.UI.height }); 
     
      echartInstance.on("click", function (params) {

          if($this.UI.isDrill !== true){
            if(this.onClick && typeof this.onClick ==='function' ){
              this.onClick(name);
            }
            return;
      ...
      if($this.UI.currentCity){
        $this.goto($this.UI.currentCity);
      }else if($this.UI.currentProvince){
        $this.goto($this.UI.currentProvince);
      }else{
